...
If (!(Test-Path 'g:\Main Courthouse\CourtTechnology\SPA_INFO\Script_Repository')){
	net use g: \\court.fresno\fsc
} Else {
	Write-Host "Drive is Mapped Already"
}
# Checks for AD and Exchange Modules are loades.
#& 'G:\Main Courthouse\CourtTechnology\SPA_INFO\Script_Repository\Powershell\Loadout\Admin-Check_and_AD-EX-Modules-Check.ps1'
#requires -Version 2 
...
$uFullName= Show-InputBox -Prompt "What is the new Users First and Last Name? Example: John Smith"
$uEmpID= Show-InputBox -Prompt "What is the new Users Last Four of Social?  Example: 9999"
$uEmpHD= Show-InputBox -Prompt "What is the new Users Hire Date. Example M/D/YYYY"
$uTitle= Show-InputBox -Prompt "Enter Users Title"
$uDepartment= Show-InputBox -Prompt "Enter Users Department"
...
If ($uADOU -eq "") {
	$adLocation= "COURT.FRESNO/--COURT USERS"
	$uADOU= "OU=--COURT USERS,DC=COURT,DC=FRESNO"
} Else {
...
-UserPrincipalName ($uName + '@COURT.FRESNO')

# add new User to default Groups
"g2810LimitedWebAccess","g2810","g2811_0900","g2810Operations" | Add-ADGroupMember -Members $uName
...
If (!(Test-Path p:)){ 
	net use p: \\Frscrtfps\s$\2810\UserFolders
} Else {
	Write-Host "Drive is Mapped Already"
}
...
$Database= Get-MailboxDatabase -Identity "Users Group 8"
...
Get-ADUser $uName -Properties * | Select @{Name="Last, First";Expression={$_.Name}},@{Name="Email";Expression={$_.EmailAddress}}, `
    ...
    Export-Csv -LiteralPath 'G:\Main Courthouse\CourtTechnology\SPA_INFO\Script_Repository\Results\NewUsers_Bulk.csv' -Append -NoTypeInformation -Force
